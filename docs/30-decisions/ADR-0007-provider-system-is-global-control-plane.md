...
- retry policy and fail-closed stream failure
...
- model routing, retry, and failure behavior become testable
...
The first implementation milestones MUST include provider profile resolution, model aliasing, credential references, provider-slot leases, normalized stream events, retry event emission, and fail-closed stream failure recording.
