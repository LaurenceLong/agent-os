...
use agent_os_sys::*;
use agent_os_thread::{
    LlmApiStyle, OpenAiModelClient, RuntimeConfig, RuntimeRunReport, ThreadRuntime,
};
...
pub(crate) fn run_chat(options: &ChatOptions) -> AgentOsResult<Value> {
    ...
    let mut client_builder = OpenAiModelClient::new(api_key, model.clone())
    ...
    let mut session = ChatSession::new(kernel, options.workspace.clone(), model);
...
fn resolve_api_key(options: &ChatOptions) -> AgentOsResult<String> {
...
impl ChatSession {
    fn new(kernel: Kernel, workspace: std::path::PathBuf, model: String) -> Self {
        Self {
            kernel,
            workspace,
            model,
    ...
    fn process_task(
        ...
        let agent = self.kernel.spawn_agent(SpawnAgentInput {
            ...
            local_goal: task_text.to_string(),
