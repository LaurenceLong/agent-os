...
use agent_os_sys::{
    CommandDefinition, ImportedAgentProfile, InstructionDocument, McpResourceDefinition,
    McpResourceTemplateDefinition, McpToolDefinition, SkillDefinition, ToolDescriptor,
};
...
pub(super) fn from_state(state: &KernelState) -> EcosystemProjection {
    ...
    let mut mcp_resources: Vec<_> = state.mcp_resources.values().cloned().collect();
    mcp_resources.sort_by(|left, right| {
        left.server_name
            .cmp(&right.server_name)
            .then_with(|| left.uri.cmp(&right.uri))
    });

    let mut mcp_resource_templates: Vec<_> =
        state.mcp_resource_templates.values().cloned().collect();
    mcp_resource_templates.sort_by(|left, right| {
        left.server_name
            .cmp(&right.server_name)
